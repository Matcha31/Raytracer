...
public:
    Camera(const Point3& position, const Point3& target, const Vector3& upVector, float fovx, float fovy, float zmin) : position(position), target(target), upVector(upVector), fovx(fovx), fovy(fovy), zmin(zmin){
